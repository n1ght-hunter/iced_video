...
impl VideoPlayer {
    ...
    pub fn new<F>(
        ...
        live: bool,
        callback: F,
    ) -> Result<(VideoPlayer, std::sync::mpsc::Receiver<GSTMessage>), Error>
    where
        F: FnMut(&AppSink) -> Result<gst::FlowSuccess, gst::FlowError> + Send + 'static,
    {
        ...
        let (sender, receiver) = mpsc::channel::<GSTMessage>();
